...
CofferGate의 Google Cloud 기반 Control Plane과 Devnet 데모 실행 흐름을 구현합니다. 데모에서는 실제 자산을 이동하지 않고, 정책을 통과한 Proposal을 Cloud KMS 서명 Attestation으로 검증합니다.
...
CLOUD_TASKS_QUEUE='demo-attestation'
...
CLOUD_KMS_KEY_VERSION='projects/project/locations/location/keyRings/ring/cryptoKeys/demo-attestation/cryptoKeyVersions/1'
```
...
`REPOSITORY_MODE=firestore`와 `DATA_MODE=live`를 함께 사용하면 서버 시작 전에 Firestore, Devnet 운영 지갑, 데모 토큰 계정, Jupiter 가격 조회, Cloud KMS 및 Cloud Tasks 필수 설정을 모두 검증합니다. 누락된 통합 설정이 있으면 런타임을 시작하지 않습니다.
...
Devnet Runtime의 IAM·Secret·KMS·Cloud Tasks Queue와 Cloud Run 설정은 다음 스크립트로 적용합니다. Runtime 및 Tasks 서비스 계정과 두 Secret은 사전에 생성해야 합니다.
...
CLOUD_KMS_KEY_VERSION='projects/your-project-id/locations/asia-northeast3/keyRings/coffergate/cryptoKeys/demo-attestation/cryptoKeyVersions/1' \
...
배포 환경은 기본 `LOG_LEVEL=info`로 요청과 Proposal 생성·Attestation 결과를 구조화 로그로 기록합니다. `x-coffergate-task-token` Header는 로그에서 자동으로 마스킹됩니다.
...
- `POST /internal/v1/demo-attestations/:proposalId` (승인된 Proposal의 Cloud KMS Attestation 생성)
